...
FalcoMode::FalcoMode(socd::SocdType socd_type, FalcoModeOptions options) {
    _socd_pair_count = 4;
...
void FalcoMode::HandleSocd(InputState &inputs) {
    _horizontal_socd = inputs.left && inputs.right;
    InputMode::HandleSocd(inputs);
}

void FalcoMode::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
    outputs.b = inputs.b;
    outputs.x = inputs.x;
    outputs.y = inputs.y;
...
void FalcoMode::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
