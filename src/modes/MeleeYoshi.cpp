...
MeleeYoshi::MeleeYoshi(socd::SocdType socd_type, MeleeYoshiOptions options) {
    _socd_pair_count = 4;
...
void MeleeYoshi::HandleSocd(InputState &inputs) {
    _horizontal_socd = inputs.left && inputs.right;
    InputMode::HandleSocd(inputs);
}

void MeleeYoshi::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
...
void MeleeYoshi::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
    ...
    bool shield_button_pressed = inputs.l || inputs.r || inputs.z || inputs.midshield;
