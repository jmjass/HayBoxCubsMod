...
extern bool recal;
bool GetOriginFromGCCInput(gc_origin_t &_origin);
void __no_inline_not_in_flash_func(GamecubeConsole::WaitForPollStart)() {
    ...
    while (true) {
        ...
        switch ((GamecubeCommand)received[0]) {
            ...
            case GamecubeCommand::RECALIBRATE:
            case GamecubeCommand::ORIGIN:
                // If recalibration command sent
                if(recal){
                    // Wait for origin to be recieved
                    while (!GetOriginFromGCCInput(updated_gc_origin)) {
                        tight_loop_contents();
                    }
                    // Wait for stop bit before responding.
                    busy_wait_us(reply_delay);
                    joybus_send_bytes(&_port, (uint8_t *)&updated_gc_origin, sizeof(gc_origin_t));   
                }else{
                    // Wait for stop bit before responding.
                    busy_wait_us(reply_delay);
