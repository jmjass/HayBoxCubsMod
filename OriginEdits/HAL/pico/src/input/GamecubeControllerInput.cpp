...
bool GamecubeControllerInput::ResetOrigin(){
...
bool GamecubeControllerInput::GetOriginFromGCC(gc_origin_t &_origin) {
    if (_controller->bOriginReceived)
    {
        _origin = _controller->origin;
        _origin.initial_inputs.r_analog = 0;
        _origin.initial_inputs.l_analog = 0;
        return true;
    }
    else
    {
        _origin = default_gc_origin; //don't use this if you're using an OEM gcc!!
        return false;
    }
}
